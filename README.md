...
- 订单查看：已支付订单列表展示、订单详情
- 密码安全：后端使用 bcryptjs 加密，前端无需处理密码加密
...
- **地址管理**：前端已实现地址的完整增删改查（调用后端相应 API）。
- **订单列表删除**：前端中的删除操作仅用于界面上移除展示项（不调用后端删除订单），后端保留订单记录用于审计与历史查询。
- **用户模块**：支持注册与登录，会话通过 cookie/session 管理；用户信息修改接口暂未开放。
